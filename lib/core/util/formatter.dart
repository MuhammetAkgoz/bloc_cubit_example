import 'package:bloc_cubit_example/core/util/currency.dart';
...
class NeoCurrencyTextInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    } else if (RegExp('[^0-9.,]').allMatches(newValue.text).isNotEmpty) {
      return oldValue;
    ...
    /// Handle numbers
      ...
      if ((!newValue.text.contains('.')) && oldValue.text.contains('.')) {
        var tempString = newValue.text.replaceAll(_numberSymbol, '');
      ...
      var newString = _formatter.format(newNumber);
  ...
  double _getTwoDecimalAfterPoint(double number) {
    ...
    final afterPoint = _getAfterPoint(temp);
  ...
  String _getAfterPoint(String text) {
    ...
    var afterPoint = text.substring(index + 2, text.length);
    return afterPoint;
  }
}
    ...
    controller = OTPTextEditController(
      codeLength: 6,
        ...
        (code) {
          ...
          /// todo: Otp code number must be provide parametric
          final exp = RegExp(r'(\d{6})');

          /// Retrieve sms code
          if (smsCodeListener != null) {
            smsCodeListener?.call(exp.stringMatch(code ?? '') ?? '');
          }
